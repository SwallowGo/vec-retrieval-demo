...
### 2.1 与真实向量数据库 / RAG 的关系（设计边界）

本项目的「DB 持久化 + 内存索引 + 全量精确检索」是向量数据库的教学简化版，真实 Agent/RAG 项目通常由专用向量库（Milvus/Qdrant/pgvector）承担检索：

| 能力 | 本项目 | 真实向量库 |
|---|---|---|
| 向量存储 | H2 VARBINARY | 列式/文件 + WAL |
| 内存索引 | ImmutableMap 快照 | HNSW / IVF-PQ 图（同样常驻内存） |
| 检索算法 | 精确全量遍历 O(N) | ANN 近似检索 O(log N) |
| 元数据过滤 | 渠道过滤 | 标量+向量混合索引 |

本形态在真实世界的对应物：FAISS 等 in-process 向量索引、从 DB 加载 embedding 进内存检索的小规模 RAG 应用。适用边界约万级文档；更大规模需替换 `ReadyIndexService` 为向量库客户端（HNSW 索引）。由于检索抽象在 `SearchService` 之后、索引通过 `EmbeddingCompletionListener` 解耦，替换实现不影响业务层架构。
...
| 2026-08-19 | 新增 §2.1「与真实向量数据库/RAG 的关系」：说明本方案与向量库的对应关系、适用边界（万级文档）与替换演进路径 | 用户追问本设计与真实 Agent/RAG 项目的差异 |
